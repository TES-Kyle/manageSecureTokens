...
echoFunc () {
    ...
    Title="manageSecureTokens"

    # Header string function
    fHeader () { echo $(fDateTime) $(hostname) $Title; }

    # Check for the log file
    if [ -e "/Library/Logs/manageSecureTokens.log" ]; then
        echo $(fHeader) "$1" >> "/Library/Logs/manageSecureTokens.log"
    else
        touch "/Library/Logs/manageSecureTokens.log"
        if [ -e "/Library/Logs/install$macOSname.log" ]; then
            echo $(fHeader) "$1" >> "/Library/Logs/manageSecureTokens.log"
        else
...
# Takes 3 args, first being the passphrase, second the salt and last the hash
...
#echoFunc "5: ${5}, 6: ${6}, 7: ${7}"
#Sends passphrase, salt and hash to the dcryption and returns the password
addAdminUserPassword=$(DecryptString ${5} SALT HASH)



#echoFunc "Admin Pass: $addAdminUserPassword"
...
# Check if the admin provided exists on the system
		if [[ $("/usr/sbin/dseditgroup" -o checkmember -m $addAdminUser admin / 2>&1) =~ "Unable" ]]; then
  		...
  		addAdminUserType="AllGood"
			...
			echoFunc "Admin user status: LIES! it did not exist go check the data" && exit 20      
        else
        echoFunc "Admin user status: You where right! the account did exists"
fi
# Check if our admin has a Secure Token
    	...
    	adminToken="false"
  		...
  		echoFunc "Admin Token: $adminToken"
# Check if $addAdminUser is actually an administrator
		...
		if [[ $("/usr/sbin/dseditgroup" -o checkmember -m $addAdminUser admin / 2>&1) =~ "yes" ]]; then
                    ...
                    echoFunc "Admin Account Status: $AdminUserType"
<<comment
#Fixing the admin to make it admin		
        ...
        echoFunc "Admin Promo status: It wasnt admin but now it is"
		else
        echoFunc "Admin Promo status: No Action Needed "
        fi
comment
...
# Check Secure Tokens Status - Do we have any Token Holder?
		...
		echoFunc "Token Status $tokenStatus"
				...
				# Check Token status for end user
			    	...
			    	userToken="false"
			  		...
			  		echoFunc "User Token: $userToken"

				# If both end user and additional admin have a secure token

				if [[ $userToken = "true" && $adminToken = "true" ]]; then
				echoFunc "All is good!"
				exit 0
				fi

				# Prompt for password
                
				echoFunc "Prompting ${userName} for their login password."
				userPass="$(/usr/bin/osascript -e 'Tell application "System Events" to display dialog "Please enter your laptop password" default answer "" with title "Laptop Password" with text buttons {"Ok"} default button 1 with hidden answer' -e 'text returned of result')"
								...
								if [ "$passDSCLCheck" -eq 0 ]; then
								    echoFunc "Password OK for $userName"
								fi
                ...
                #echoFunc "Admin password after token: $addAdminUserPassword"
               
				echoFunc "Token granted to end user!"
				...
				echoFunc "Token granted to both additional admin and end user!"
				...
				echoFunc "End user admin token holder granted token to additional admin!"
				...
				echoFunc "Houston we have a problem!"
				...
				##echoFunc "End user promoted to admin!"
				...
				##echoFunc "End user demoted back to standard!"	
				#exit 1
				fi
